...
def pd_controller(x_p, y_p, z_p, yaw_a, dx_p, dy_p, dz_p, dyaw_a, KP_X, KP_Y, KP_Z, KP_YAW, KD_X, KD_Y, KD_Z, KD_YAW):
    ...
    if controller_on_or_off == 1.0:
        if count == 0.0:
            ...
            vel_msg.linear.x = now_error_x * KP_X + ((now_error_x - previous_error_x) / (1.0/30)) * KD_X
            vel_msg.linear.y = now_error_y * KP_Y + ((now_error_y - previous_error_y) / (1.0/30)) * KD_Y
            ...
            vel_msg.linear.z = now_error_z * KP_Z + ((now_error_z - previous_error_z) / (1.0/30)) * KD_Z
            ...
            vel_msg.angular.z = -(now_error_yaw * KP_YAW + ((now_error_yaw - previous_error_yaw) / (1.0/30)) * KD_YAW)
